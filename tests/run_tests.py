...
def run_demo() -> int:
    ...
    code |= _run(["bash", "-n", "scripts/backup.sh"], cwd=REPO_ROOT)
    code |= _run(["bash", "-n", "scripts/restore.sh"], cwd=REPO_ROOT)
    code |= _run(["bash", "-n", "scripts/check_replication.sh"], cwd=REPO_ROOT)
    code |= _run(["bash", "-n", "scripts/seed_demo_data.sh"], cwd=REPO_ROOT)
    code |= _run(["bash", "-n", "scripts/migrate_safe_add_column.sh"], cwd=REPO_ROOT)
    code |= _run(
...
def run_production() -> int:
    ...
    env = dict(os.environ)
    env.setdefault("COMPOSE_PROJECT_NAME", "repo15_prodtests")
